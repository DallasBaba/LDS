...
erDiagram
  ...
  FACT_DRIVER_MONTHLY_METRICS {
    string driver_id  %% composite business key, resolved to SK at ETL
    int driver_sk FK
    string month
    int trips_completed
    decimal total_miles
  ...
  FACT_TRUCK_UTILIZATION {
    string truck_id  %% composite business key, resolved to SK at ETL
    int truck_sk FK
    string month
    int trips_completed
    decimal total_miles
...
Add your preferred license (e.g., MIT) if you plan to share this publicly.
